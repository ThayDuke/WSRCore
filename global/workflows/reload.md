...
## Bước 3: Đọc Router & Trạng thái Checkpoint & Quy trình Brainstorm
- Đọc hướng dẫn: `AGENTS.md` tại gốc dự án (nay nằm trong `.agents/AGENTS.md`) để nhận diện các lệnh tắt.
- **Cơ chế nạp trì hoãn (Lazy Loading workflows & rules)**: Mặc định khi reload, Agent KHÔNG nạp nội dung của các file quy tắc đặc thù (`AG_DECISION_RULES.md`), playbook hay checklist để tiết kiệm token. Agent chỉ nạp và ghi nhớ Router trong `AGENTS.md` và nạp thêm file quy trình brainstorm `.agents/global/workflows/br.md` để luôn sẵn sàng phản xạ theo quy chuẩn. Khi bắt đầu thực hiện các nhiệm vụ chuyên sâu hoặc dùng lệnh khác, Agent mới tự động nạp tri thức tương ứng theo yêu cầu.
...
Chỉ nạp các quy tắc cốt lõi toàn cục, quy trình brainstorm và checkpoint để khôi phục nhanh ngữ cảnh:
- Các file ở Bước 1, Bước 2, Bước 3.
- Khối nén ngữ cảnh: `.agents/memory/project_checkpoint.yaml` (nếu có).
- *Không nạp* các file quy tắc cục bộ (`AG_DECISION_RULES.md`), file hỗ trợ gỡ lỗi (`dec-debug-playbook.md`), checklist (`regression-checklists.md`), các file `SKILL.md` và `AG_LESSONS.jsonl`.
...
Nạp các quy tắc cốt lõi (Cấp độ Tiêu chuẩn) + `AG_DECISION_RULES.md` + các tài liệu hỗ trợ gỡ lỗi và kiểm thử cục bộ:
- Các playbook gỡ lỗi cục bộ trong `.agents/rules/` (ví dụ: `dec-debug-playbook.md` hoặc tương đương).
...
- Tất cả quy tắc, checkpoint và tài liệu hỗ trợ (Playbook, Checklist, Mẫu prompt, `AG_DECISION_RULES.md`).
